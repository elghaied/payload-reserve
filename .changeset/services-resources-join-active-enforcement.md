...
- **BREAKING** `active: false` on a Service or Resource is now enforced at booking time — creating a reservation against an inactive service/resource (or any multi-resource `items[]` entry referencing one), or updating a reservation to newly reference one, is rejected; inactive services/resources are also excluded from availability. This only affects new or changed references: an unchanged `(service, resource)` pair is not re-checked, so an existing booking stays fully editable and cancellable even after its service or resource is deactivated later. Set `enforceActive: false` in the plugin config to restore the previous behaviour, where `active` was purely a display flag with no effect on booking or availability.
