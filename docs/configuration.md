...
| `calendar` | `ReservationCalendarConfig` | `undefined` | Calendar presentation: default view, hidden view tabs, first day of the week and per-status colour overrides (see sub-options) |
| `calendar.defaultView` | `ReservationCalendarViewMode` | `'month'` | View tab the calendar opens on. Validated at init; must not also be in `hiddenViews` |
...
| `calendar.mobileDefaultView` | `ReservationCalendarViewMode` | `calendar.defaultView` | View tab the calendar opens on when the viewport is ≤768px (Payload's `s` breakpoint — the width its admin nav collapses at). Applied once on first render; rotating a tablet later never changes the view. Validated like `defaultView` |
...
| `calendar.weekStartsOn` | `0 \| 1 \| 2 \| 3 \| 4 \| 5 \| 6` | `0` (Sunday) | First day of the week in the month and week grids and the availability overview — `1` for Monday. Day headers rotate to match. Validated at init: must be an integer 0–6 |
