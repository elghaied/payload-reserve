...
| `enforceActive` | `boolean` | `true` | Reject bookings against an inactive service or resource (or an `items[]` entry referencing one), and exclude inactive services/resources from availability. Set `false` to restore the previous behaviour, where `active` was advisory only and had no effect on booking or availability |
