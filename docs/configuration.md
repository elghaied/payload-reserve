...
| `enforceActive` | `boolean` | `true` | Reject creating a reservation against an inactive service or resource (or an `items[]` entry referencing one), or updating a reservation to newly reference an inactive one; exclude inactive services/resources from availability. Only new or changed references are checked — an unchanged `(service, resource)` pair is not re-validated, so an existing booking stays fully editable and cancellable after its service or resource is deactivated later. Set `false` to restore the previous behaviour, where `active` was advisory only and had no effect on booking or availability |
