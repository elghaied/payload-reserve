...
**Dates follow the admin language.** Every date and time label across the Calendar, Reservation Detail drawer, Availability Overview, Availability Time Field and Dashboard widget is formatted in the admin's current language (`useTranslation().i18n.language`, e.g. `fr` → "sam. 12 sept.", `en` → "Sat, Sep 12") rather than the runtime's default locale. This matters because the calendar is server-rendered: Node's locale is not the browser's, and formatting with the runtime default produced a React hydration mismatch for any non-en-US browser. The `formatReservationTime` / `formatReservationDateLabel` helpers (exported from `payload-reserve/client`) take an optional trailing `locale` for the same reason — pass `i18n.language` from custom components too. The plugin's own components run the language through `safeLocale` first (a host-registered language key that is not valid BCP 47, such as `en_GB`, would otherwise make every `Intl` formatter throw); it falls back to `en`.
